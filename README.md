...
1. Download the latest `GroundControl Setup x.x.x.exe` from [Releases](https://github.com/itsela-ms/GroundControl/releases)
2. Run the installer — it installs to your user profile and adds a Start Menu entry
3. Launch GroundControl from the Start Menu
...
GroundControl checks for updates automatically on startup. When a new version is available:
...
5. Upload these files from `release/`:
   - `GroundControl Setup X.Y.Z.exe`
   - `GroundControl Setup X.Y.Z.exe.blockmap`
